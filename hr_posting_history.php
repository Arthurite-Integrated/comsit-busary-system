...
<div id="page" class="container">
		<div class="content">
                ...
                <h3><i>My Posting History</i></h3>
			<p>
          <div class="easyui-tabs" style="width:auto;height:auto"> <!-- begining of main tab-->
                       <div title="Posting" style="padding:10px">  <!-- posting tab  -->
                         <?php
						   $r=@strtolower($r_vals);
						     
							 $sql="select * from hr_postingtb where fileno='$login_id' order by post_date desc";
							   
							$res_v=@mysqli_query($con, $sql);
							$sn=0;
							$tb="<table align='center'><tr><th>S/NO</th><th>DEPARTMENT</th><th>UNIT</th><th>DATE POSTED</th><th>POSITION</th></tr>";
						  if(@mysqli_num_rows($res_v)>=1)
						   {
								while($rs_v=@mysqli_fetch_array($res_v))
								 {
									 ++$sn;
									 $tb.="<tr><td>$sn</td><td>".@get_dept_name($rs_v['post_dept'])."</td><td>".@get_unit_name($rs_v['post_dept'],$rs_v['post_unit'])."</td><td>".@date('d/m/Y',strtotime($rs_v['post_date']))."</td><td>{$rs_v['position']}</td></tr>";
								 } //end of while
								 
								 $tb.="</table>"; echo $tb;
						    }
							else
						       echo "<font color='red'><b>No posting record to display</b></font>";
						 ?>
