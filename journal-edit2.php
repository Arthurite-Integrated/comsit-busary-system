...
<!-- begining of main tab-->
  ...
  <?php
	   $r=@strtolower(base64_decode($_REQUEST['r_val']));
		...
		$sql="SELECT t.*, concat(s.surname, ' ', s.first_name, ' ', s.other_name) AS sname FROM journaltb t INNER JOIN stafftb s ON t.entry_by=s.fileno WHERE t.journal_date >= '$from' AND t.journal_date <= '$to' AND t.pvno NOT LIKE '%TSA%' AND t.journalno != '' ORDER BY t.journalno,  t.journal_date desc";
		...
		<thead>
		<tr><th><input type='checkbox' id='checkAll' name='checkAll' value='{$rs_v['journalno']}'></th><th>S/NO</th>
                    <th>JV. NO.</th><!--th>ITEM DESC.</th-->
	  ...
	  if(@mysqli_num_rows($res_v)>=1)
	   {
			while($rs_v=@mysqli_fetch_array($res_v))
			 {
				 ...
				 $tb.="<tr {$color}><td><input type='checkbox' class='checkboxes' id='pv_jv_{$rs_v['id']}' name='pv_jv[]' value='{$rs_v['id']}'></td><td>$sn</td>
                                         <td>$pvno</td>
				 <!--td>{$item_desc}</td-->
				 ...
				 //$tb.="<a href=\"javascript:sendRequest('editPostedJournal2', 'EDIT', '$tid');\">EDIT</a> | ";
				 //$tb.="<a href=\"javascript:sendRequest('editPostedJournal2', 'DELETE', '$tid');\">DELETE</a>";
                     $tb .= "</td--></tr>";
			...
			YEAR: <select id="tyear" name="tyear">
				...
				<button type="button" class="btn btn-outline-primary btn-fw" name="toledger" id="toledger" onclick="sendRequest('savePostedJournal2');">MOVE SELECTION TO GENERAL LEDGER</button>
			 <div id="savePostedJournal2"></div>
			 <?php
		}
		else
