...
{   //swap content begins where cv means div id name
 ...
 if(cv=='main_login') //start putme_login
  ...
  if(cv=='process_voucher') //start putme_login
  {
	...
	if(cv=='process_voucherpv') //start putme_login
  {
		$.post(url,$("#frmpro").serialize()+"&contentvar="+cv,function(data){  //ajaxfile/scriptfile_a is called undernith
			$(divid).html(data).show(); //report result from the ajaxfile, the data stores the information to be displayed from ajaxfile
			$("#roll").html('').show();
                         ...
                         <?php
						  ...
						  //echo $sql="select * from vouchertb where dept_code='".$_SESSION['userunit']."' and prepared_by = '".$login_id."' and checked_by ='' order by voucher_date desc";exit;
						   ...
						   elseif($r=="super admin" or $r=="checked by officer" or $r=="administrator")
						       $sql="select * from vouchertb where (dept_code='".$_SESSION['userunit']."' or prepared_by = '".$login_id."') and checked_by='' order by voucher_date desc";
						   elseif($r=="super admin"  or $r=="administrator" or $r=="authorized officer")//or $r=="authorized officer"
						       $sql="select * from vouchertb where dept_code='".$_SESSION['userunit']."' and checked_by!='' and checked_action='Approved' and authorized_by ='' order by voucher_date desc";
                        ...
                        <?php
						   $r=@strtolower($r_vals);
						    if($r=="super admin" or $r=="checked by officer" or $r=="administrator")
						       $sql="select * from vouchertb where dept_code='".$_SESSION['userunit']."' and checked_by!='' order by voucher_date desc";
						   elseif($r=="super admin" or $r=="authorized officer")
						       $sql="select * from vouchertb where dept_code='".$_SESSION['userunit']."' and authorized_by != '' order by voucher_date desc";
							   elseif($r=="super admin" or $r=="final authorized officer")
						  ...
						  // elseif($r=="super admin" or $r=="bursar")
						   ...
						   elseif($r=="super admin" or $r=="cash officer")
							...
							$res_v=@mysqli_query($con, $sql);
							$sn=0;
							$tb="<table id='MyTable2' class='display; align='left' border='1' cellpadding='5' cellspacing='5' rules='cols' frame='box'>
							<thead> 
							<tr style='border:solid 1px #000; background-color:#f2f2f2'><th>S/NO</th><th>NARRATION</th><!--th>PV NO</th--><th>PAYEE</th><th>PAYEE ACCT NO.</th><!--th>PAYEE BANK</th--><th>DATE</th><th>GROSS (NET)</th><th>CHECKED</th><th>CERTIFIED</th><th>CONTROLLED</th><th>AUDITED</th><th>PAID</th><th>ACTION</th></tr></thead><tbody>";
						  if(@mysqli_num_rows($res_v)>=1)
						   {
								while($rs_v=@mysqli_fetch_array($res_v))
								 ...
								 $paid_date=$rs_v['date_paid'];			$paid_remark=$rs_v['paid_remark'];
									...
									//$tb.="<tr><td>$sn</td><td>$pvno</td><!--<td>$pvno_paid</td>--><td>$payee_name</td><td>$payee_acct_no</td><td>$payee_bank_name</td><td>".date('d/m/Y',strtotime($voucher_date))."</td><td><a class='iframe' href='voucher_report.php?p=$p'>VIEW</a></tr>";
									 ...
									 $tb.=" | <a href=\"javascript:swapcontent('display_voucher_process','$pvno','$r_vals');\">PROCESS</a>| <a href=\"javascript:swapcontent('display_voucher_processpv','$pvno','$r_vals');\">Edit PV</a></td></tr>";
								 } //end of while
                        ...
                        <?php
						   $r=@strtolower($r_vals);
						    if($r=="super admin" or $r=="prepared officer" or $r=="administrator")
						       $sql="select * from vouchertb where dept_code='".$_SESSION['userunit']."' and prepared_by='".$login_id."' and checked_action = 'Queried' order by voucher_date desc";
						    elseif($r=="administrator" or $r=="budget officer")
						       $sql="select * from vouchertb where prepared_by='".$login_id."' and checked_action = 'Queried' order by voucher_date desc";
							elseif($r=="super admin" or $r=="checked by officer" or $r=="administrator")
						       $sql="select * from vouchertb where dept_code='".$_SESSION['userunit']."' and (checked_by!='' and checked_action = 'Queried') or (controlled_by != '' and controlled_action = 'Queried') or (audit_by != '' and audit_action = 'Queried') or (authorized_by != '' and authorized_action = 'Queried') or (paid_by != '' and paid_action = 'Queried') order by voucher_date desc";
						   elseif($r=="super admin" or $r=="authorized officer")
						       $sql="select * from vouchertb where dept_code='".$_SESSION['userunit']."' and (authorized_by != '' and authorized_action = 'Queried') or (controlled_by != '' and controlled_action = 'Queried') or (audit_by != '' and audit_action = 'Queried') or (paid_by != '' and paid_action = 'Queried') order by voucher_date desc";
						   elseif($r=="super admin" or $r=="expenditure control")
