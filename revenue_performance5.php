...
<title>FINAL ACCOUNT REVENUE REPORT</title>
...
//////////////////////////////////////////////////////Report Body /////////////////////////////////////////////
?>
<?php echo "<center><img src='$val[1]' width='50' height='50' style='float:center'/><!--<img src='images/uith.png' width='50' height='50' style='float:right'/>--></center><b><p align='center'><u>".strtoupper($val[0])."<br/>"."FINAL ACCOUNT REVENUE REPORT AS AT:   ".$from."</u></p></b><hr><p>";
  ?>
                                                                                                                                                                                                                                                                              <center>
  ...
  <tr>
    ...
    <th> AMOUNT FOR <?php echo $year ?><br>&#8358;</th>
   
	</tr>

 <?php
 	$bcat= mysqli_query($con, "SELECT DISTINCT folio_code FROM transtb where folio_code != '' and rev_code != '' and year(transdate)='$year'");
	$total_b = 0; 		$total_s = 0;		$total_v = 0;  $sn=1;	
	...
	// $sql_folio=@mysqli_query($con, "select distinct rev_code,reven_title from revenue_code where category = '".$yq[0]."' order by rev_code");
	  ...
	  $folio_name=get_folio_name($folio_code);
	 ...
	 // $sub_total_b += $folio_budget;			$total_b += $sub_total_b;
	  ...
	  $folio_budget_generate1=folio_budget_generate6($folio_code,$year);
	  
	  //$folio_budget_generate2= explode('***',$folio_budget_generate1);
	  $folio_budget_generate = $folio_budget_generate1;
	  //$folio_recipt = $folio_budget_generate2[1];
	  $total_s += $folio_budget_generate;	
	...
	//  $get_var_balance= get_var_balance($folio,$year);
	 // $sub_total_v += $get_var_balance;		$total_v += $sub_total_v;
		  ...
		  echo "<td> $sn </td>
				<td>$folio_code</td>
				<td nowrap height='25'>$folio_name</td>
