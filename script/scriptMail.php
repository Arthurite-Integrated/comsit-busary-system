<?php
     ...
     if($requestID=="getnextmemo")
     {
          $sql="SELECT * FROM memo_movementtb where dept_unit='".mysqli_real_escape_string($con, $_REQUEST['dept_unit'])."' and memo_status='IN'";
	$r=mysqli_query($con, $sql);
	$cnt = mysqli_num_rows($r) + 1;
          $q = mysqli_query($con, "SELECT * FROM unittb WHERE unit_code='".mysqli_real_escape_string($con, $_REQUEST['dept_unit'])."'");
          $r=mysqli_fetch_array($q, MYSQLI_BOTH);
          $dept_unit = strtoupper($r['unit_name']);
          
	$dpt = explode(' ', $dept_unit);
	$mnt=strtoupper(date('M'));
	$sc=array(',', "'");
	$dpt = str_replace($sc, "", $dpt[0]);
