...
?>
    ...
    <th><?php echo strtoupper(substr($month_name[$month-1],0,3))." - ".strtoupper(substr($month_name[$montht-1],0,3)).' '.$year; ?> TOTAL<br>&#8358;</th>
    <th>VARIANCE<br>&#8358;</th>
	</tr>
 ...
 <?php
	$monthly_var=array(); 
	  ...
	  $bcat= mysqli_query($con, "SELECT DISTINCT bursary_sub_category FROM budgettb WHERE budget_year='".$year."' and bursary_sub_category != '' and (bursary_category = 'Recurrent' OR bursary_category = 'Departmental')");// ORDER BY bursary_sub_category ASC");
	...
	while($yq =  mysqli_fetch_array($bcat, 3 )){
		...
		$sql_folio=@mysqli_query($con, "select distinct folio_code, budget_title from budgettb where budget_year = $year and bursary_sub_category = '".$yq[0]."' order by budget_title ASC");
