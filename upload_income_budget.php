...
<div id="tooplate_wrapper">
    ...
    <div id="tooplate_main">
        ...
        <div id="content_title_box">
	        <h2>Upload Revenue Income Budget</h2>
                <p>Use this page to Upload Revenue Income</p>
        ...
        <div id="tooplate_content">
        	...
        	<div class="content_box">
                ...
                <div class="easyui-tabs" data-options="tabWidth:100,tabHeight:60" style="width:700px;" id="tt">
                  <div title="<span class='tt-inner'><img src='images/newmail.png'/><br>Upload Budget</span>" style="padding:10px">
  ...
  -webkit-border-radius: 5px;'>
      ...
      <table width="80%" >
        <tr>
          <td ><a href="template/revenue_budet_template.csv">Budget Samplee</a></td>
          <td><a href="template/revenue_budet_template.csv">Budget Sample</a></td>
        </tr>
        <tr>
        ...
        <tr>
          <td colspan="4" align="center"><div align="center">
             <input name="mode" type="hidden" id="mode" value="upload" /> <input name="button" type="submit" class="btn" id="button" value="Upload Income Budget For Revenue" />
            </div>
  ...
  <?php
		      ...
		      if(isset($_REQUEST['mode']) and $_REQUEST['mode']=='upload')
		   {
			...
			$login_id=@$_SESSION['login_id'];
			   ...
			   $thead="<table><tr><th>S/NO</th><th>REVENUE CODE</th><th>REVENUE TITLE</th><th>AMOUNT</th><th>STATUS</th></tr>";
					 ...
					 $file_array = @file("$uploadFile");
					    while (list($line_num, $line) = each($file_array)) 
							{ // each line
								...
								$rev_code = @trim($fileRow[1]); 
								
								$amount = @addslashes(@trim($fileRow[2])); 
								$year = @addslashes(@trim($fileRow[3])); 
								
								//////////////////////check folio code//////////////////////////
							
							$res_lf=@mysqli_query($con, "select reven_title from revenue_code where rev_code='".$rev_code."'");
			  $rs_lf=@mysqli_fetch_array($res_lf);
							 $rev_title = $rs_lf['reven_title'];
			  	if(@mysqli_num_rows($res_lf)>=1)
				
				{$fon = "UPLOADED";} else {$fon = "NOT UPLOADED";}
			  				///////////////////////////////////////////////////////////////
							
							 $res_l=@mysqli_query($con, "select * from budget_income where rev_code='".$rev_code."' and amount='".$amount."' and  year='".$year."'");
			  $rs_l=@mysqli_fetch_array($res_l);
			  if(@mysqli_num_rows($res_l)>=1)
			  {
				  mysqli_query($con, "delete from budget_income where rev_code='".$rev_code."' and amount='".$amount."' and  year='".$year."'");
				  $tbody.="<tr><td>$sn</td><td>$rev_code</td><td>$rev_title</td><td>$amount</td><td>$fon</td></tr>";
			  } elseif (( mysqli_num_rows($res_l) <=0 and $fon == "UPLOADED")) {
			 	////let us add them to the necessary tables  ".$receiptno."
 mysqli_query($con, "insert into budget_income set rev_code='".$rev_code."',amount='".$amount."',year='".$year."',entry_date=CURDATE(), entry_by='$login_id'");
						
									
									$tbody.="<tr><td>$sn</td><td>$rev_code</td><td>$rev_title</td><td>$amount</td><td><font color='#00FF00'>$fon</font></td></tr>";
									
								} else { //end of if added to jamb table
								$tbody.="<tr><td>$sn</td><td>$rev_code</td><td>$rev_title</td><td>$amount</td><td><font color='#FF0000'>$fon</font></td></tr>";
								}
