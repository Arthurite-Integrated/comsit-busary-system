...
<div id="tooplate_wrapper">
        ...
        <div id="tooplate_content">
        	...
        	<div class="content_box">
                ...
                <div class="easyui-tabs" data-options="tabWidth:100,tabHeight:60" style="width:700px;" id="tt">
                  <div title="<span class='tt-inner'><img src='images/newmail.png'/><br>Upload Receipt/Payment</span>" style="padding:10px">
  ...
  -webkit-border-radius: 5px;'>
      ...
      <table width="80%" >
        <tr>
          <td ><a href="template/receipt template.csv">Receipt Sample</a></td>
          <td><a href="template/payment template.csv">Payment  Sample</a></td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td width="58%"><select name="account" id="account" class="txt" style="width: 300px"  >
        ...
        <tr>
          <td colspan="4" align="center"><div align="center">
             <input name="mode" type="hidden" id="mode" value="upload" /> <input name="button" type="submit" class="btn" id="button" value="Upload Payment or Receipt" />
            </div>
					 ...
					 $file_array = @file("$uploadFile");
					    while (list($line_num, $line) = each($file_array)) 
							{ // each line
								...
								$date_entry = @trim($fileRow[0]); 
								$chequeno = @trim($fileRow[1]); 
								
								$pvno = @addslashes(@trim($fileRow[2])); 
								$folio_code = @addslashes(@trim($fileRow[3])); 
								$payee = @addslashes(@trim($fileRow[5]));
								$amount = @addslashes(@trim($fileRow[6]));
								$transtype = @addslashes(@trim($fileRow[7]));
								//////////////////////check folio code//////////////////////////
                                ...
                                //if(@mysqli_num_rows($res_lf)>=1) {$fon = "UPLOADED";} else {$fon = "NOT UPLOADED";}
                                            ///////////////////////////////////////////////////////////////
                            if($folio_code != '' && $folio_code != 'CODE'){
                                $sqll="select * from transtb where acctcode='".$account."' and folio_code='".$folio_code."' and  pvno='".$pvno."' and transtype='".$transtype."' and transdate='".$date_entry."' and amount=".$amount."";
                                $res_l=@mysqli_query($con, $sqll);
                                $rs_l=@mysqli_fetch_array($res_l);
                                if(@mysqli_num_rows($res_l)>=1)
                                {
                                     mysqli_query($con, "delete from transtb where acctcode='".$account."', folio_code='".$folio_code."', pvno='".$pvno."',transtype='".$transtype."', transdate='".$date_entry."', amount=".$amount."");
                                    //$tbody.="<tr><td>$sn</td><td>$date_entry</td><td>$pvno</td><td>$folio_code</td><td>$amount</td><td>$transtype</td></tr>";
                                } 
                                ///*if /*(( mysqli_num_rows($res_l) <=0 and $fon == "UPLOADED"))*/ 
                                ////let us add them to the necessary tables  ".$receiptno."
                                    $sql="insert into transtb set dept_acctcode='', acctcode='".$account."', pvno='".$pvno."',payee='".$payee."',chequeno='".$chequeno."', folio_code='".$folio_code."', transtype='".$transtype."', transdate='".$date_entry."', amount=".$amount.",entry_date=CURDATE(), entry_time=CURTIME(), entry_by='$login_id'";
                                    // mysqli_query($con, $sql);
                                    if( mysqli_query($con, $sql)) {$fon = "UPLOADED";} else {$fon = "NOT UPLOADED";}
                                    ...
                                    $tbody.="<tr><td>$sn</td><td>$date_entry</td><td>$pvno</td><td>$folio_code</td><td>$amount</td><td>$transtype</td><td><font color='#00FF00'>$fon</font></td></tr>";
                                        ...
                                        /*} else { //end of if added to jamb table
                                        $tbody.="<tr><td>$sn</td><td>$date_entry</td><td>$pvno</td><td>$folio_code</td><td>$amount</td><td>$transtype</td><td><font color='#FF0000'>$fon</font></td></tr>";
                                        */
					 ...
					 $file_array = @file("$uploadFile");
					    while (list($line_num, $line) = each($file_array)) 
							{ // each line
								...
								$fileRow = @explode(",",$line);
								$date_entry = @trim($fileRow[0]); 
								$folio_code = @addslashes(@trim($fileRow[1])); 
								$receiptno = @addslashes(@trim($fileRow[2]));
								$amount = @addslashes(@trim($fileRow[3]));
								$transtype = @addslashes(@trim($fileRow[4]));
								
								$res_lf=@mysqli_query($con, "select folio_code from foliotb where folio_code='".$folio_code."'");
			  $rs_lf=@mysqli_fetch_array($res_lf);
			  	if(@mysqli_num_rows($res_lf)>=1) {$fon = "UPLOADED";} else {$fon = "NOT UPLOADED";}
							
								 $res_l=@mysqli_query($con, "select * from transtb where acctcode='".$account."' and folio_code='".$folio_code."' and  receiptno='".$receiptno."' and transtype='".$transtype."' and transdate='".$date_entry."' and amount=".$amount."");
			  $rs_l=@mysqli_fetch_array($res_l);
			  if(@mysqli_num_rows($res_l)>=1)
			  {
				   mysqli_query($con, "delete from transtb where acctcode='".$account."', folio_code='".$folio_code."', receiptno='".$receiptno."',transtype='".$transtype."', transdate='".$date_entry."', amount=".$amount."");
				  $tbody.="<tr><td>$sn</td><td>$date_entry</td><td>$receiptno</td><td>$folio_code</td><td>$amount</td><td>$transtype</td></tr>";
			  } elseif (( mysqli_num_rows($res_l) <=0 and $fon == "UPLOADED")) {
			 	////let us add them to the necessary tables  ".$receiptno."
							 mysqli_query($con, "insert into transtb set dept_acctcode='', acctcode='".$account."', folio_code='".$folio_code."', receiptno='".$receiptno."',transtype='".$transtype."', transdate='".$date_entry."', amount=".$amount.",entry_date=CURDATE(), entry_time=CURTIME(), entry_by='$login_id'");
						
									
									$tbody.="<tr><td>$sn</td><td>$date_entry</td><td>$receiptno</td><td>$folio_code</td><td>$amount</td><td>$transtype</td><td><font color='#00FF00'>$fon</font></td></tr>";
									
								} //end of if added to jamb table
								else {
									$tbody.="<tr><td>$sn</td><td>$date_entry</td><td>$receiptno</td><td>$folio_code</td><td>$amount</td><td>$transtype</td><td><font color='#FF0000'>$fon</font></td></tr>";
									}
							} //end of file loop per row
