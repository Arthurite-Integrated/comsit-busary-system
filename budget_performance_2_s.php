...
$year=$_REQUEST['pyear2']; $month=$_REQUEST['pmonth2'];
$montht=$_REQUEST['pmontht2'];
//$budget_cat[] =  mysqli_real_escape_string($con, $_POST['pcat']);
$budget_cat='';
foreach ($_POST['pcat'] as $selectedOption)
    $budget_cat.="'".$selectedOption."',";
	//$budget_cat.="''";
	$budget_cat= substr($budget_cat, 0, -1);
	//echo $budget_cat;
	$catTitle=trim(str_replace("'", "", $budget_cat));
...
?>
 ...
 <?php
	$monthly_var=array(); 
	$sql="SELECT DISTINCT bursary_sub_category FROM budgettb WHERE budget_year='".$year."' and bursary_sub_category in ({$budget_cat})";
	...
	while($yq =  mysqli_fetch_array($bcat, 3 )){
		...
		echo "<tr bgcolor='#999'>";
		echo "<th colspan='5' align='left'>".strtoupper($yq_n)."</th>";
		echo '</thead><tbody>';
		echo "</tr>";
		$sn=0;	
