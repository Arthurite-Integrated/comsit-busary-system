...
{   //swap content begins where cv means div id name
 ...
 if(cv=='main_login') //start putme_login
  ...
  if(cv=='display_grn_process') //start putme_login
  {
	//  alert(cv+" "+v+" "+a);exit();
	 // alert($("frmlogin").serialize());exit();
		//$.post(url,$("frmlogin").serialize()+"&contentvar="+cv,function(data){  //ajaxfile/scriptfile_a is called undernith
		$.post(url,{contentvar:cv,grn:v,r_vals:a},function(data){
  ...
  if(cv=='process_grn') //start putme_login
  {
	  //alert(cv+" "+v+" "+a);exit();
	  //alert($("form").serialize());exit();
			...
			<p>
                <div class="easyui-tabs" style="width:800px;height:auto" > <!-- begining of main tab-->
                       <div title="Pending" style="padding:10px">  <!-- pending tab  -->
                         <?php
						   $r=@strtolower($r_vals);
						    if($r=="super admin"  or $r=="administrator" or $r=="fixed asset" )
						       $sql="select * from grn_sivtb where checked_by = 'No' order by entry_date desc ";
							$res_v=@mysqli_query($con, $sql);
							$sn=0;
							$tb="<table align='left' border='1' cellpadding='5' cellspacing='5' rules='rows' frame='box'><tr><th>S/NO</th><!--<th>GRN</th><th>PV NO</th><th>SIV</th>--><th>GRN</th><th>SIV</th><th>DATE</th><th>ACTION</th></tr>";
						  if(@mysqli_num_rows($res_v)>=1)
						   {
								while($rs_v=@mysqli_fetch_array($res_v))
								 {
									 ++$sn;
									 $grn=$rs_v['grn']; //$pvno_paid=$rs_v['pvno_paid'];
									 $p=base64_encode($grn);
									 $siv=$rs_v['siv'];
									 $entry_date=$rs_v['entry_date'];
									 $tb.="<tr><td>$sn</td><td>$grn</td><!--<td>$pvno_paid</td><td>$payee_name</td><td>$payee_acct_no</td>--><td>$siv</td><td>".date('d/m/Y',strtotime($entry_date))."</td><td><a class='iframe' href='grn_report.php?p=$p' >VIEW</a> | <a href=\"javascript:swapcontent('display_grn_process','$grn','$r_vals');\">PROCESS</a></td></tr>";
						       ...
						       echo "<font color='red'><b>No GRN to process</b></font>";
						 ?>
                       ...
                       <div title="Processed" style="padding:10px"> <!-- processed tab  -->
                        <?php
						   $r=@strtolower($r_vals);
							...
							$res_v=@mysqli_query($con, $sql);
							$sn=0;
							$tb="<table align='left' border='1' cellpadding='5' cellspacing='5' rules='rows' frame='box'><tr><th>S/NO</th><th>GRN</th><!--<th>PV NO</th><th>PAYEE</th><th>PAYEE ACCT NO.</th>--><th>SIV</th><th>DATE</th><th>ACTION</th></tr>";
						  if(@mysqli_num_rows($res_v)>=1)
						   {
								while($rs_v=@mysqli_fetch_array($res_v))
								 {
									 ...
									 $siv=$rs_v['siv'];
									 $entry_date=$rs_v['entry_date'];
									 $tb.="<tr><td>$sn</td><td>$grn</td><!--<td>$pvno_paid</td><td>$payee_name</td><td>$payee_acct_no</td>--><td>$siv</td><td>".date('d/m/Y',strtotime($entry_date))."</td><td><a class='iframe' href='grn_report.php?p=$p'>VIEW</a> <!--| <a href=\"javascript:swapcontent('display_grn_process','$pvno','$r_vals');\">PROCESS</a>--></td></tr>";
								 } //end of while
				   ...
				   <div id="display" class="easyui-window" title="GRN Process" data-options="
                   modal:true,
                   closed:true,
                   iconCls:'icon-save',
			onResize:function(){
