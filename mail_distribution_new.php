...
if($login_status=='staff')
			...
			//echo $role."=$login_id=>$login_status<br>123";
				$res_r=@mysqli_query($con, "select * from users_roletb where fileno='$login_id' and status='Active' and role like '%Budget and Expenditure%'") or die ( mysqli_error($con));
    ...
    <div id="tooplate_main">
        ...
        <div id="tooplate_content">
			...
			$dept_unit = $role_use ;
			 ...
			 //exit;
				  // $res_c=@mysqli_query($con, "select m.memo_id,m.memo_status,m.memo_from,m.description,m.amount,m.datein, mm.memo_id, mm.memo_status, mm.dept_unit from memotb m ,memo_movementtb mm  where m.memo_status = 'In Progress' and mm.dept_unit = '$dept_unit' and mm.memo_status = 'IN' order by m.memo_id");
	 $res_c=@mysqli_query($con, "select memo_id, memo_status from memo_movementtb where dept_unit = '$dept_unit' and memo_status = 'IN'");
	  $c =  mysqli_num_rows($res_c);
				   while($rs_c=@mysqli_fetch_array($res_c))
				    {
						$id=@$rs_c['memo_id'];	
						
						
		$res_c2=@mysqli_query($con, "select * from memotb where memo_id = '$id' and memo_status = 'In Progress'");
				while($rs_c2=@mysqli_fetch_array($res_c2))
				    {
				
						$memo_from=@$rs_c2['memo_from'];	
						$memo_id=@$rs_c2['memo_id'];
						$description=@$rs_c2['description'];	
						$amount=@$rs_c2['amount'];	
						$datein=@$rs_c2['datein'];	
						
						echo "<tr><td>$memo_id</td><td>$memo_from</td><td>$description</td><td>$amount </td><td>$datein </td><td> <input type='checkbox' name='movement' value = $memo_id></td>";
					
					$res_c3=@mysqli_query($con, "select memo_id from memo_assigntb where memo_id = '$memo_id'");
					$num_result =  mysqli_num_rows($res_c3);
					if ($num_result>0)
								{
									$stat = 'Assigned';
										} else {  $stat = 'Not Assigned'; }
				...
				<tr> <th>Select Staff : </th><th colspan="3"> <select name="staff" id="staff" >
							<option value="" selected>Select Staff...</option>';
							 <?php  $res_s=@mysqli_query($con, "select * from stafftb where category='Non-Academic' order by surname,fileno");
	   ...
	   echo "<option value='$fileno'>$fullname</option>";
							  ...
							  ?>
						</select></th><th><input type="hidden" id="login_id" name="login_id" value="<?php echo $login_id; ?>"/><input type="hidden" id="staff_category" name="staff_category" value="<?php echo $role; ?>"/></th>
                  <th><input type='button' name='button' id='button' value='Send' class="btn"  onClick="swapcontent('assignmail',$('#staff').val(),$('#movement[]').val(),$('#login_id').val(),$('#staff_category').val());"/></th><td>&nbsp;</td></tr>  
				  ...
				  <tr>
                              <td colspan="6"><div id="assignmail"></div></td>
                            </tr>
                  </table>
                  </p>
                      </form>
                    ...
                    <div title="<span class='tt-inner'><img src='images/outmail.png'/><br>Outgoing Mail</span>" style="padding:10px" >
                    <table id="dgout" title="" style="width:680px;" data-options="
                singleSelect:true,
                url: 'scriptfile_y.php?contentvar=incoming_mail',
                rownumbers:true,method:'get',toolbar:'#toolbar', pagination:true,
                pageSize:10
            ">
        <thead>
            <tr>
