...
<div id="tooplate_wrapper">
    ...
    <div id="tooplate_main">
        ...
        <div id="content_title_box">
	        <h2>Upload Asset Balance</h2>
                <p>Use this page to Upload Asset Balance</p>
        ...
        <div id="tooplate_content">
        	...
        	<div class="content_box">
                ...
                <div class="easyui-tabs" data-options="tabWidth:100,tabHeight:60" style="width:700px;" id="tt">
                  <div title="<span class='tt-inner'><img src='images/newmail.png'/><br>Asset Balance</span>" style="padding:10px">
  ...
  -webkit-border-radius: 5px;'>
      ...
      <table width="80%" >
        <tr>
          <td ><a href="template/asset_balance_upload.csv">upload Sample</a></td>
          <td><a href="template/asset_balance_upload.csv">upload Sample</a></td>
        </tr>
        <tr>
          ...
          <td width="58%"><select name="utype" id="utype" style="width:200px">
            ...
            <option value="Budget">Asset Balance</option>
        ...
        <tr>
          <td colspan="4" align="center"><div align="center">
             <input name="mode" type="hidden" id="mode" value="upload" /> <input name="button" type="submit" class="btn" id="button" value="Upload Asset Balances" />
            </div>
  ...
  <?php
		      ...
		      if(isset($_REQUEST['mode']) and $_REQUEST['mode']=='upload')
		   {
			...
			$login_id=@$_SESSION['login_id'];
			   ...
			   $thead="<table><tr><th>S/NO</th><th>ASSET NAME</th><th>AMOUNT</th><th>STATUS</th></tr>";
			   if (@move_uploaded_file($_FILES['file']['tmp_name'], $uploadFile))
					 ...
					 $file_array = @file("$uploadFile");
					    while (list($line_num, $line) = each($file_array)) 
							{ // each line
								...
								$asset_name = @trim($fileRow[0]); 
								$amount = @addslashes(@trim($fileRow[1])); 
								$year = @addslashes(@trim($fileRow[2])); 
				...
				$res_l=@mysqli_query($con, "select * from asset_balance where asset_name='".$asset_name."' and amount='".$amount."' and  year='".$year."'");
			  $rs_l=@mysqli_fetch_array($res_l);
			  if(@mysqli_num_rows($res_l)>=1)
			  {
				  mysqli_query($con, "delete from asset_balance where asset_name='".$asset_name."' and amount='".$amount."' and  year='".$year."'");
				  //$tbody.="<tr><td>$sn</td><td>$asset_name</td><td>$amount</td><td>$fon</td></tr>";
			  } elseif (( mysqli_num_rows($res_l) <=0 )) {
			 	////let us add them to the necessary tables  ".$receiptno."
				  
if ( mysqli_query($con, "insert into asset_balance set asset_name='".$asset_name."',amount='".$amount."',year='".$year."',entry_date=CURDATE(), entry_by='$login_id'"))
{
	$fon = "UPLOADED";
} else {
	$fon = "NOT UPLOADED";
									...
									$tbody.="<tr><td>$sn</td><td>$asset_name</td><td>$amount</td><td><font color='#00FF00'>$fon</font></td></tr>";
									
								} else { //end of if added to jamb table
								$tbody.="<tr><td>$sn</td><td>$asset_name</td><td>$amount</td><td><font color='#FF0000'>$fon</font></td></tr>";
								}
