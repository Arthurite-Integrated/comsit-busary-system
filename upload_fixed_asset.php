...
<div id="tooplate_wrapper">
        ...
        <div id="tooplate_content">
        	...
        	<div class="content_box">
                ...
                <div class="easyui-tabs" data-options="tabWidth:100,tabHeight:60" style="width:700px;" id="tt">
                  <div title="<span class='tt-inner'><img src='images/newmail.png'/><br>Upload Asset</span>" style="padding:10px">
  ...
  -webkit-border-radius: 5px;'>
      ...
      <table width="61%" >
        <tr>
          <td width="15%">Asset Category:</td>
          <td width="58%"><select name="cat_type" id="cat_type" style="width:200px"  onChange="swapcontent('catdiv', $('#cat_type').val());">
          ...
          <td width="17%"><div id="catdiv">
            </div>
           <input name="mode" type="hidden" id="mode" value="upload" /></td>
        </tr>
        <tr>
        ...
        <tr>
          <td colspan="4" align="center"><div align="center">
              <input name="button" type="submit" class="btn" id="button" value="Click here to upload" />
            </div>
  ...
  <?php
		   ...
		   if(isset($_REQUEST['mode']) and $_REQUEST['mode']=='upload')
		   {
			  ...
			  $asset_cat=@$_REQUEST['asset_cat'];
			   ...
			   $thead="<table><tr><th>S/NO</th><th>ASSET NO</th><th>ASSET NAME</th><th>ASSET DATE</th><th>LOCATION</th><th>AMOUNT</th></tr>";
					 ...
					 $file_array = @file("$uploadFile");
					    while (list($line_num, $line) = each($file_array)) 
							{ // each line
								...
								$identify_string = @trim($fileRow[0]); 
								$descritption = str_replace(",","",@addslashes(@trim($fileRow[1]))); 
								$descritption = str_replace('"','',@addslashes(@trim($fileRow[1]))); 
								
						$location = str_replace(",","'",@addslashes(@trim($fileRow[2]))); 
								$qty = @addslashes(@trim($fileRow[3]));
								$grn = @addslashes(@trim($fileRow[4])); $acq_date = @addslashes(@trim($fileRow[5])); $amount = @addslashes(@trim($fileRow[6]));
								
							
								////let us add them to the necessary tables
								if( mysqli_query($con, "insert into assettb
 set prod_id='$descritption',descritption='$descritption',identify_string='$identify_string',acq_date='$acq_date',location='$location',amount='$amount',asset_type = '$asset_cat',asset_category = '$cat_type',grn='$grn',siv='$grn',fix_con = 'Yes',qty='$qty',entry_by='$login_id',entry_date =CURDATE(),entry_time = CURTIME()"))
								{
									
	$res_d2=  mysqli_query($con, "select * from useful_lifetb where asset_type='$cat_type'"); 
 ...
 mysqli_query($con, "insert into asset_depreciation set identify_string='$identify_string',cost_bf='$amount',acc_depr='$depreciation',no_years='$percs',entry_date=CURDATE(),entry_time=CURTIME(),entry_by='$login_id'");			
									...
									$tbody.="<tr><td>$sn</td><td>$identify_string</td><td>$descritption</td><td>$acq_date</td><td>$location</td><td>$amount</td></tr>";
	...
	$tbody.="<tr><td>$sn</td><td>$identify_string</td><td>$descritption</td><td>$acq_date</td><td>$location</td><td><font color='#FF0000'>Duplicate</font></td></tr>";
								}
