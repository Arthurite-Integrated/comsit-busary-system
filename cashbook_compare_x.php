...
<!-- begining of main tab-->
  ...
  <?php
     if(isset($_POST['mode']) && $_POST['mode']=='upload'){
		...
		$sq="SELECT id, pvno, amount, rmonth AS mon, ryear AS yea, batchno FROM recon_remitatb WHERE rmonth='{$imonth}' AND ryear='{$iyear}' AND (pvno LIKE '%{$pvno}') AND paytype='Debit'";
	...
	$sq="SELECT id, pvno, amount, rmonth AS mon, ryear AS yea FROM recon_remitatb WHERE rmonth='{$from}' AND ryear='{$to}' AND pvno  NOT IN ({$pvno_array}) AND paytype='Debit' ORDER BY pvno";
               $mq=mysqli_query($con, $sq);
