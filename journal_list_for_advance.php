...
{   //swap content begins where cv means div id name
 ...
 if(cv=='voucher_section_entry') //start putme_login
  ...
  if(cv=='save_pre_jvno') //start unit
  {
	  var pv_id = v;
	  var pvno=$("#schpvno"+pv_id).val();
	  var upvno=$("#adpvno").val();
	  var fdate=$("#fdate").val();
	  var pyear=$("#schYear").val();
          ...
          if(upvno=='') {
                    alert("Select PV from list.");
                    exit;
          }
          $("#schdiv"+pv_id).html('<img src="images/loader.gif" width="100" height="100" alt="loading">').show();
          $.post(url,{contentvar:cv, pvno:pvno, id:pv_id, fdate:fdate, year:pyear, jvno:a, upvno:upvno},function(data){
                    $("#roll").html('').show();
...
<div id="tooplate_wrapper">
    ...
    <div id="tooplate_main">
        ...
        <div id="content_title_box">
	        <h2>Journal Correction (Advances)</h2>
	        ...
	        <form name="frm" id="frm" method="post" action="<?=$_SERVER['PHP_SELF'];?>">
                    ...
                    <div class="row">
                              ...
                              - 
                                        ...
                                        <input type="submit" id="btn" name="btn" value="SEARCH" class="btn">
                              </div><hr>
                    </div>
                ...
                <div id="display2"></div>
                    ...
                    <div style="padding:10px"> <!-- QUERIED tab  -->
                           <?php
                              $dFrm = $_POST['dFrm'];
                              $dTo = $_POST['dTo'];
                              $year = date('Y', strtotime($_POST['dFrm']));
                              echo "<input type='hidden' id='fdate' name='fdate' value='{$dFrm}'>";
                              echo "<input type='hidden' id='schYear' name='fdate' value='{$year}'>";
                              $postYear = date('y', strtotime($_POST['schYear']."-01-01"));
                              $r=@strtolower($r_vals);
                              

                              if(isset($_POST['dFrm']) && $_POST['dFrm']!='' && isset($_POST['dTo']) && $_POST['dTo']!=''){
                                        echo "<h2>LIST OF JOURNALS POSTED BETWEEN {$dFrm} AND {$dTo}</h2>";

                                        $r=@strtolower($r_vals);							
                                            $yr = date('Y');
                                            $sql="SELECT v.* FROM vouchertb v INNER JOIN voucher_folio_codetb vf ON v.pvno=vf.pvno WHERE YEAR(voucher_date) = '{$year}' AND vf.folio_code='09-001-3087' order by voucher_date desc";
                                         $res_v=@mysqli_query($con, $sql);
                                        if(@mysqli_num_rows($res_v)>=1)
                                        {
                                        echo "<h4>ADVANCE VOUCHER LIST FOR YEAR {$year}</h4>";
                                         $sn=0;
                                         $tb="<table id='MyTable3' class='table display' align='left' border='1' cellpadding='5' cellspacing='5' rules='cols' frame='box'>
                                         <thead> 
                                         <tr style='border:solid 1px #000; background-color:#f2f2f2'><th></th><th>S/NO</th><th>PROCESS NO</th><th>PV NO</th><th>AMOUNT</th><th>PAYEE</th><th>PAYEE BANK</th><th>DATE</th><th>ACTION</th></tr></thead><tbody>";
                                         while($rs_v=@mysqli_fetch_array($res_v))
                                         {
                                             ...
                                             $tb.="<tr><td><input type='radio' value='{$pvno}' id='adpv{$pvno}' name='adpv' onclick='$(\"#adpvno\").val($(this).val());'></td><td>$sn</td><td>$pvno</td><td>$pvno_paid</td><td>".$amount."</td><td>$payee_name</td><td>$payee_bank_name</td><td>".date('d/m/Y',strtotime($voucher_date))."</td>
                                             <td nowrap><a class='iframe' href='voucher_report.php?p=$p' >VIEW</a>";
                                             $tb.="</td></tr>";
                                         } //end of while
                                         
                                         $tb.="</tbody></table>
                                         <input type='hidden' value='' id='adpvno' name='adpvno'>"; echo $tb;
                                        }
//----------------------------------------------------------------------------------------------------------------------             
                                        
                                        $sql="SELECT * FROM journaltb WHERE journal_date BETWEEN '{$dFrm}' AND '{$dTo}' AND dept_code NOT IN ('11') order by journalno, journal_date desc";
                              }
                              ?>
                    <hr><div>
        </form>  
                              ...
                              <thead> 
                              <tr style='border:solid 1px #000; background-color:#f2f2f2'><th>S/NO</th><th>JOURNAL NO</th><th>PV NO</th><th>ACTION</th><th>AMOUNT</th><th>PAYEE</th><th>FACULTY/CENTER</th><th>DATE</th></tr></thead><tbody>";
                              if(@mysqli_num_rows($res_v)>=1)
                              {
                                    while($rs_v=@mysqli_fetch_array($res_v))
                                    {
                                        ++$sn;
                                        $journalno=$rs_v['journalno']; 
                                        $payee_name=$rs_v['payee_name'];
                                        $payee_acct_no=$rs_v['payee_acct_no'];
                                        $payee_bank_name=$rs_v['payee_bank_name'];
                                        $voucher_date=$rs_v['journal_date'];
                                        $pre_pvno_paid = $rs_v['pvno'];
                                        $fact=$bursary->get_any_value('unit_name', 'unittb', 'unit_code', @$rs_v['dept_code']);
                                        
                                        $amount = number_format($rs_v['cr_amount'], 2);
                                        $yr = $postYear; //date('y'); //date('y', strtotime($prepared_date));
                                        $tb.="<tr><td>$sn</td><td>$journalno</td>
                                        <td><div id='schdiv{$rs_v['id']}'>$pre_pvno_paid</div></td>
                                        <td>
                                        <input type='hidden' id='schid{$rs_v['id']}' name='schid[]' value='{$rs_v['id']}'>
                                        <input type='hidden' id='jvnos{$rs_v['id']}' name='jvnos[]' value='{$journalno}'>
                                        <input type='text' placeholder='PV NO' id='schpvno{$rs_v['id']}' name='schpvno[]' value='' size='15'>
                                        <input type='button' id='schbtn{$rs_v['id']}' name='schbtn[]' value='OK' onclick='swapcontent(\"save_pre_jvno\", \"{$rs_v['id']}\", \"{$journalno}\");'>
                                        </td>
                                        <td>".$amount."</td>
                                        <td>$payee_name</td>
                                        <td>$fact</td>
