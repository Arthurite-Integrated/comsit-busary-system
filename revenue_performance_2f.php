...
//////////////////////////////////////////////////////Report Body /////////////////////////////////////////////
?>
<?php echo "<center><img src='$val[1]' width='50' height='50' style='float:center'/><!--<img src='images/uith.png' width='50' height='50' style='float:right'/>--></center><b><p align='center'><u>".strtoupper($val[0])."<br/>"."SPREADSHEET REVENUE REPORT AS AT:   ".get_month_name($month). "&nbsp; $year" ."</u></p></b><hr><p>";
  ?>
	...
	$k=false;
	 ...
	 $sql_folio=@mysqli_query($con, "select distinct rev_code,reven_title from revenue_code where category = '".$yq[0]."' order by reven_title");
	   while($rst_folio=@mysqli_fetch_array($sql_folio))
	   {
		   ...
		   $sqll3sx = "select folio_code from revenue_code where rev_code = '$rev_code' limit 1 "; 
	  	$msql3sx= mysqli_query($con, $sqll3sx) or die( mysqli_error($con));
	$accodesx = mysqli_fetch_array($msql3sx, 3 );
		   
			 $fcode  = $accodesx['folio_code'];
		   
		   $folio_name=get_folio_name($fcode);
           ...
           $reven_title=$rst_folio['reven_title'];
			...
			echo "<td> $sn </td>
			<td>$reven_title</td>
			<td >$rev_code ($fcode) ($folio_name)</td>";
			$i=0;	
