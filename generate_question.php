...
<div id="tooplate_wrapper">
    ...
    <div id="tooplate_main">
        ...
        <div id="content_title_box">
	        <h2>Generate Question Types</h2>
                <p>Use the form below to generate question types</p>
        ...
        <!--	<div class="content_box">-->
              ...
              <div class="cleaner"></div>
			   <div id="contact_form">
			    <form method="post" action="question_type.php" enctype="multipart/form-data" name="cform" id="cform" onSubmit="return myVal()" target="_blank">
			   		<label for="author">Session:</label><select name="session"  id="session" class="required input_field">
						...
						<label for="author">Type:</label><select name="type"  id="type" class="required input_field">
          							<option value="" selected>--</option>
						...
						<div class="cleaner h10"></div>
						<label for="author">Number of questions to be generated:</label> <input type="text" id="no_of_question" name="no_of_question" class="required input_field" />
						<div class="cleaner h10"></div>
						
					
					<input type="hidden" name="mode" value="process" />
                                <input type="submit" class="submit_btn float_l" name="btn_generate" id="btn_generate" value="Generate" />
                                <input type="submit" class="submit_btn float_l" name="btn_print" id="btn_print" value="Re-print Question" />
                                <input type="submit" class="submit_btn float_l" name="btn_print_answer" id="btn_print_answer" value="Print Answer" />	
								<input type="submit" class="submit_btn float_l" name="btn_delete" id="btn_delete" value="Delete Generated Questions" />	
			    ...
			    if (obj.type.value == "") { alert("Type is compulsary"); obj.type.focus(); return false; }
                ...
                var postConf = window.confirm("Are you sure you want to perform this operation? \n\n Click 'Ok' to proceed if yes, otherwise click 'Cancel' to discontinue");
              ...
              //  $ccode = @strtoupper(@str_replace(" ","",@trim($myLine[0])));
				...
				if (@mysqli_num_rows($courseInfo) >= 1)
				  ...
				  { //not already allocated
					 ...
					 if(@mysqli_query($con, "insert into $courses_table set question_id='$question_id',question_detail='$question',correct_answer='$answer',point='$point',session='$session',entry_date=CURDATE(),entry_time=CURTIME(),entry_by='$entryby'"))
					 {
	...
	//TINY.box.show({html:content2,boxid:'frameless',width:750,height:450,fixed:true,maskopacity:40,closejs:function(){closeJS()}})
			...
			*/
				...
				$rs=@mysqli_query($con, "select * from question_banktb where session='$session'");
				$n=0;$question_list="";
