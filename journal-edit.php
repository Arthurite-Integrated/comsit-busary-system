...
<script>
     function sendRequest(requestID, loaderDiv='', recID=''){
          ...
          if(requestID=="editPostedJournal"){
			if(loaderDiv=="DELETE") {
				if(!confirm('You are about to delete the selected Jounal Entry. \nClick OK to confirm Delete.')){
					$("#movetoledger").html('<p><b>Delete Canceled.</b></p>').show();
					exit;
				}
			}
               $("#movetoledger").html(loader).show();
               $.post(url, {requestID:requestID, action:loaderDiv, trans_id:recID}, function(response){
			//alert(recID);
                    if(loaderDiv=="DELETE") {
			$("#movetoledger").html(response);
			...
			$("#display").window("open");
			$("#movetoledger").html('').show();
		}
               });
          }

	if(requestID=='savePostedJournal'){
		if($("#pvno22").val()=='' || $("#amount").val()=='' || $("#amount").val()==0 || $("#folio").val()==''){
			alert('Check entry, empty fields not allowed!');
			exit;
		}
		$.post(url, $("#update_trans").serialize()+"&requestID="+requestID, function(data){ 
			$("#savePostedJournal").html(data).show();
			document.location.reload();
		});
	}
     }
...
<div id="tooplate_wrapper">
    ...
    <div id="tooplate_main">
        ...
        <div id="content_title_box">
	        <h2>Edit Posted Journal Entries</h2>
...
<!-- begining of main tab-->
  ...
  <?php
	   $r=@strtolower(base64_decode($_REQUEST['r_val']));
		...
		$sql="SELECT t.*, concat(s.surname, ' ', s.first_name, ' ', s.other_name) AS sname FROM transtb t INNER JOIN stafftb s ON t.entry_by=s.fileno WHERE t.transdate >= '$from' AND t.transdate <= '$to' AND t.pvno NOT LIKE '%TSA%' AND t.pvno != '' ORDER BY t.pvno,  t.transdate desc";
		//distinct pvno, transdate, amount, dept_acctcode, acctcode
		$res_v=@mysqli_query($con, $sql);
		$sn=0;
		$tb="<table align='left' border='1' cellpadding='5' cellspacing='0' rules='rows' frame='box' width='100%' class='display' id='dTable'> 
		<thead>
		<tr><th><input type='checkbox' id='checkAll' name='checkAll' value='{$rs_v['pvno']}'></th><th>S/NO</th>
                    <th>PV./JV. NO.</th><th>ITEM DESC.</th>
                    <th>ACCOUNT</th><th>TRANS. TYPE</th><th>AMOUNT</th><th>PREPARED BY</th><th>FACULTY/CENTRE</th><th>DATE</th><th>ACTION</th></tr></thead><tbody>";
	  if(@mysqli_num_rows($res_v)>=1)
	   {
			while($rs_v=@mysqli_fetch_array($res_v))
			 {
				 ++$sn;
                                         $tid=$rs_v['id'];
				 $pvno=$rs_v['pvno'];			
                                         $project=get_dept_name($rs_v['dept_acctcode']);
				 //$transid=$rs_v['chequeno'];	
				 ...
				 $dcode = $bursary->get_any_value("folio_code", "journal_folio_codetb", "journalno", $pvno, " AND trans_type='Debit'");
                 $dcode = explode('-', $dcode);
                 $debit_code = $dcode[1];
                 $unit = $bursary->get_any_value("department_name", "account_departments", "department_code", $debit_code);
				 $color = ""; 
				 if(($rs_v['folio_code']!="09-001-3087" && $rs_v['folio_code']!="02-001-3087") && $rs_v['transtype']=="Credit") $color = " style='background-color: pink;' ";
				 $tb.="<tr {$color}><td><input type='checkbox' class='checkboxes' id='pv_jv_{$rs_v['id']}' name='pv_jv[]' value='{$rs_v['id']}'></td>
				 <td>{$sn}</td>
                     <td>";
				 if($rs_v['transtype']=="Debit"){
				 	$tb.="<input type='hidden' id='hpv_jv_{$rs_v['id']}' name='hpv_jv[]' value='{$pvno}'>
						<input type='text' id='tpv_jv_{$rs_v['id']}' name='tpv_jv[]' value='{$pvno}'>
						{$pvno}";
				 }else $tb.="{$pvno}";
				 $tb.="</td>
				 <td>{$item_desc}</td>
				 <td>".get_account_name($acct, false)."</td>
				 <td>{$rs_v['transtype']}</td>
				 ...
				 $tb.="<a href=\"javascript:sendRequest('editPostedJournal', 'EDIT', '$tid');\">EDIT</a> | ";
				 $tb.="<a href=\"javascript:sendRequest('editPostedJournal', 'DELETE', '$tid');\">DELETE</a></td></tr>";
			...
			YEAR: <select id="tyear" name="tyear">
				...
				| <button type="button" class="btn btn-outline-primary btn-fw" name="bupdate" id="bupdate" onclick="sendRequest('batchUpdate');">UPDATE SELECTION</button>
  ...
  ?>
	      ...
	      <div id="display" class="easyui-window" title="Edit Posted Journal Entries" data-options="
                   modal:true,
                   closed:true,
                   iconCls:'icon-save',
			onResize:function(){
				$(this).window('hcenter');
			}" style="width:600px;height:auto;padding:10px; display:none; top:150px;"> </div>
            </p>
	  </form>
		</div>           
