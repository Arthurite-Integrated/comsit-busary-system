                    ...
                    <div style="padding:10px"> 
                              ...
                              while($fcode=mysqli_fetch_array($mq, 3)){
                                  $fac .= ", '{$fcode[0]}'";
                                        ...
                                        echo "<h2>LIST OF UNPAID VOUCHERS WITH PV.NO.</h2>";
                                        //READ VOUCHER IDs NOT FOUND IN THE PAY_SCHEDULE USING 'RIGHT JOIN'
                                        if($r=="prepared officer") 
                                            $sql="SELECT v.* FROM pv_pay_scheduletb p RIGHT JOIN vouchertb v ON p.pvid=v.id WHERE p.pvid IS NULL AND (v.pre_pvno != '' OR v.pre_pvno Is Not Null) AND (v.paid_action='' OR v.paid_action Is Null) AND dept_vou IN ({$fac}) order by v.pre_pvtype, v.pre_pvserial ASC";
                                        else 
                                            $sql="SELECT v.* FROM pv_pay_scheduletb p RIGHT JOIN vouchertb v ON p.pvid=v.id WHERE p.pvid IS NULL AND (v.pre_pvno != '' OR v.pre_pvno Is Not Null) AND (v.paid_action='' OR v.paid_action Is Null) order by v.pre_pvtype, v.pre_pvserial ASC";
                                        ?>
                                        <?php
                                        //echo $sql;
                                        ...
                                        if(@mysqli_num_rows($res_v)>=1)
                                        {
                                            ...
                                            STAFF: <select id="tstaff" name="tstaff">
                                                  ...
                                                  <?php
                                                        ...
                                                        if($r=="prepared officer") 
                                                            $r=@mysqli_query($con, "SELECT s.* from stafftb s INNER JOIN journal_code_user j ON s.fileno=j.fileno WHERE j.jvcode IN ({$fac}) AND s.status='Active' AND s.dept_code='126' AND s.fileno not like '%admin%' order by s.surname");
                                                        else
                                                            $r=@mysqli_query($con, "SELECT * from stafftb where status='Active' AND dept_code='126' AND unit_code='8' AND fileno not like '%admin%' order by surname");
                                                        while ($rcourse=@mysqli_fetch_array($r))
                                                        {
                                                            $scourse=@$rcourse['surname'].' '.$rcourse['first_name'].' '.$rcourse['other_name'];$pcode=@$rcourse['fileno'];
                                        ...
                                        DATE: <input type="date" id="tdate" name="tdate" value="" >
                                                  <button type="button" class="btn btn-outline-primary btn-fw" name="toledger" id="toledger" onclick="sendRequest('assignPVScheddule');">ASSIGN PV TO SELECTED STAFF</button>
                                        <div id="assignPVScheddule"></div>
                                                  <?php
                                        }
                                        else
