...
$(document).ready(function(){
	 $(".iframe").colorbox({iframe:true, width:"53%", height:"60%"});
});
...
<div class="content" style="width: 1250px;">
			...
			<p>&nbsp;</p>
			<h3>APPLY FILTER [SELECT MONTH/YEAR]</h3><hr>
			<div class="row">
				<div class="col-sm-12">
				<label><strong>Select Date: </strong></label> <input type="month" id="dFrm" name="dFrm" value="" class="form-control">
  ...
  <?php
	   $r=@strtolower(base64_decode($_REQUEST['r_val']));
		...
		$from = $_POST['dFrm']."-01";
		   ...
		   $sql="SELECT * from transtb where YEAR(transdate) >= '{$year}' AND MONTH(transdate) = '{$month}' AND transtype='Credit' AND receiptno!='' ORDER BY receiptno desc";
		...
		<thead>
		          <tr>
                                        <th>
                                                  <input type='checkbox' id='checkAll' name='checkAll' value='{$rs_v['pvno']}'>
                                        </th>
                                        <th>S/NO</th>
                                        <!--<th>PROJECT</th>-->
                                        <th>RECEIPT NO.</th>
                                        <!--th>CHEQUE/TRANS. ID</th-->
                                        <th>CR ACCOUNT</th>
                                        <th>ITEM DESCCRIPTION</th>
                                        <th>AMOUNT</th>
                                        <th>DATE</th>
                                        <th>ACTION</th>
                              </tr>
			...
			while($rs_v=@mysqli_fetch_array($res_v))
			 {
				 ...
				 $acct=$rs_v['acctcode'];		$amount=number_format($rs_v['amount'], 2);
				 ...
				 <td nowrap>";
				 if($receiptno!='') $tb.="<a class='iframe' href='receipt.php?p={$p}&tid={$tid}' > VIEW </a> | ";
