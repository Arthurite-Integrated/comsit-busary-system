			...
			if(v=='save')
			 {
                if($('#type option:selected').val() == 'External' && $("#bank").val()==''){
				 ...
				 if($("#pay_date").val()=='' || $("#dept").val()=='' || $("#pvno").val()=='' || $("#account").val()=='' || $("#folio").val()=='' || $("#type").val()=='' || $("#name").val()=='' || $("#act_no").val()=='' || $("#desc").val()=='' || $("#vamount").val()=='' || $("#pro_typ").val()=='')
				 {
					 alert("All compulsory fields must be filled before you can proceed");
					 $('#roll').html('').show();
					 exit();
...
<div id="tooplate_wrapper">
    ...
    <div id="tooplate_main">
        ...
        <div id="content_title_box">
	        <h2><i>Payment Entry</i></h2>
                <p>Use this form to post payment entry</p>
        </div><!-- end of content_title_box -->
   
        <div id="tooplate_content">
	        
        	<div class="content_box">
              <div class="easyui-panel" title="New Payment/Reciept Entry" style="width:800px">     
		  ...
		  <table border="0" cellpadding="0" cellspacing="0" class="vch">
		    <tr>
			  ...
			  <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-search" onclick="swapcontent('search_voucher_by_pvno');"><strong><font color="#000099">Search by PV. No.</font></strong></a> <a class='iframe easyui-linkbutton' iconCls="icon-tip" href="votebook.php"><strong><font color="#000099">Check Vote Book</font></strong></a>
...
|
<input name="pro_typ" type="hidden" id="pro_typ" size="25" value="" />
<style>
input[type="radio"] {
...
<label>
  <input name="pro_type" type="radio" id="pro_type_0" value="pending" onChange="if($('#pro_type_0:checked').length!=0){ $('#pro_typ').val($(this).val()); $('#ppvno').hide(); $('pvno_paid').val(''); }">
  Process Further</label>
 |  <label>
    <input type="radio" name="pro_type" value="final" id="pro_type_1" onChange="if($('#pro_type_1:checked').length!=0){ $('#pro_typ').val($(this).val()); $('#ppvno').show(); }">
    Final Account Entry</label>
  </strong>
<?php
				...
				echo "Memo ID : $memo_id<br>From : $from <br>Description : $description<br>Memo Amount : $amount<br>Amount Approved : $amount_approved<br>Comment : $remark<br>Created Date : $datein";*/
			...
			<select name="voucher_unit" id="voucher_unit" style="width: 300px" onchange="swapcontent('refresh',this.value);">
				  ...
				  <option value="Salary">Salary, Loans, Advances and Procurement</option>
				  <option value="Expenditure">Budget, Expenditure Control and Stores</option>
		    ...
		    <tr>
		      <th height="50" align="left" bgcolor="#F1F1F1" ><!--<div align="left">Department:<br />
              ...
              <div align="left">Account to be Debited:<br />
                <select name="account" id="account" class="txt" style="width: 300px"  >
                  ...
                  <option value="GIFMIS">GIFMIS</option>
                    <?php
                          /*$res_c=@mysqli_query($con, "select * from account_depttb order by deptname");
                          while($rs_c=@mysqli_fetch_array($res_c))
                           {
                              $acct_code=@$rs_c['dept_acctcode'];
							  $acct_name=@$rs_c['deptname'];
...
<select name="type" id="type" class="txt" onchange="swapcontent('load_voucher_fileno',this.value);swapcontent('load_voucher_details_entry',this.value); if($('#type option:selected').val() == 'Internal'){ $('#ptin_row').hide(); }else{ $('#ptin_row').show(); } " style="width: 300px">
		        <option selected="selected" value="">---</option>
