...
if(cv=='login') //start putme_login
 ...
 if(cv=='main_login') //start putme_login
  ...
  if(cv=='allowance_definition_section') //start putme_login
  {
// alert(cv+" "+v+" "+a); exit;
  	 var mydata=JSON.stringify($('#dept_frm').serializeObject()); //get all the serialize object as JSON object and decode it where you need them
	 		...
	 		if(v=='search')		
			 {
				 if($("#folio_code").val()=='')
				  {
					  alert("No search criteria provided. Select Item Code.");
					  $(divid).html('').show();	
					  $('#roll').html('').show();
					  exit();
				  }
...
<div id="tooplate_wrapper">
    ...
    <div id="tooplate_main">
        ...
        <div id="content_title_box">
	        <h2>Allowance Definition (Group)</h2>
   ...
   <div class="content">
			...
			<tr>
				...
				<select name="folio_code" id="folio_code" tabindex="1" autofocus>
					...
					<?php
					//$r=@mysqli_query($con, "select distinct * from salary_codetb where category = 'ALLOWANCE' and status='Active' order by account_code");
  ...
  <table width="100%" border="0" align="left" cellpadding="3" cellspacing="0">
				<tr class="s_cat">
				  <td style="color:#F00" colspan="2" align="left" valign="top">Please note that you are supposed to select an item from the following criteria!</td>
                  </tr>
                <tr class="s_cat">
  ...
  <table width="100%" border="0" align="left" cellpadding="3" cellspacing="0">
				<tr>		
				<th colspan="2">Monthly Amount (<?php echo '&#8358;'; ?>)<strong style="color:#F00">*</strong><br />
				...
				<br>
				<input type="button" class="btn" name="sbtn" id="sbtn" value="Save" tabindex="17" onclick="swapcontent('allowance_definition_section','save','Debit','');" />
				<input type="button" class="btn" name="chbtn" id="chsbtn" value="Search" onclick="swapcontent('allowance_definition_section','search','Debit','');" />
				<input type="button" class="btn" name="chbtn" id="chsbtn" value="View All" onclick="swapcontent('allowance_definition_section','view_all','Debit','');" />
				<input type="button" class="btn" name="chbtn" id="chsbtn" value="Refresh" onclick="swapcontent('allowance_definition_section','refresh','Debit','');" /></td>
				</tr>
				<tr>
				  <td colspan="2"><div id="allowance_definition_section"></div>
				<div id="display"></div>
				<div id="roll"></div></td>
			  </tr>
				<tr>
