...
{   //swap content begins where cv means div id name
 ...
 if(cv=='main_login') //start putme_login
  ...
  if(cv=='budget_section_exp') //start putme_login
  {
	//  alert(cv+" "+v+" "+a); //exit();
	//  alert($("form").serialize()); //exit();
...
if(cv=='budget_breakdown') //start putme_login
  ...
  if(cv=='load_code') //start putme_login
  {
	  //alert(cv+" "+v+" "+a);exit();
	 // alert($("frmlogin").serialize());exit();
		//$.post(url,$("frmlogin").serialize()+"&contentvar="+cv,function(data){  //ajaxfile/scriptfile_a is called undernith
...
<div id="tooplate_wrapper">
    ...
    <div id="tooplate_main">
        ...
        <div id="content_title_box">
	        <h2>Budget Entry (Amount Spent)</h2>
                <p>&nbsp;</p>
	...
	//FILTER/SEARCH SCRIPT STAR5T HERE FOR GRIDS
        $(function(){
			...
			<br>
			<input type="button" class="btn" onclick="swapcontent('budget_section_exp', 'search');" value=" SEARCH "> | 
			<input type="button" class="btn" onclick="swapcontent('budget_section_exp', 'save');" value=" SAVE BUDGET ">
