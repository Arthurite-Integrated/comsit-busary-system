...
// alert(cv+" "+v+" "+a); exit;
   ...
   if(cv=='posting_section2') //start putme_login
  {
// alert(cv+" "+v+" "+a); exit;
if((v=='save' || v=='update')&&($('#dept').val()=='' || $('#acctcode').val()==''  || $('#folio_code').val()=='' || $('#amount').val()=='')) //$('#transdate').val()=='' || || $('#fileno').val()=='' 
			{
    ...
    <div id="tooplate_main">
        ...
        <div id="tooplate_content">
          ...
          <div class="content">
                ...
                <h3><i>Revenue Posting</i></h3> -->
			<p>
			<form action="#" method="post" enctype="multipart/form-data" name="dept_frm" id="dept_frm">
			<center>
			<table border="0" cellpadding="5" cellspacing="0">
              <tr>
                <td align="left" valign="top"><strong>Department/Unit/Center/School</strong><br />
                <select name="dept" id="dept"  tabindex="1" style="width:400px">
<option value="" selected>--</option>
<?php 
//onChange="javascript:swapcontent('account',this.value);"
...
?>
              ...
              <tr>
                <td colspan='1' align="left" valign="top"><strong>Account</strong><br /><select name="acctcode" id="acctcode" tabindex="2" style="width:400px">
  <option value="" selected>--</option>
	  <?php
	  //<span id="account"></span>
	  // $tb.="<option value='$acctcode'>$acctname <=> ($acctcode)</option>"; 
	  $res_a=@mysqli_query($con, "select * from bank_accounttb where status='Active' order by acctname");
  ...
  <tr>
<td colspan='1' align="left" valign="top"><strong>Date</strong><br />
...
<input type="date" min="<?=$lockdate; ?>" name="transdate" id="transdate" size="25" tabindex="3" value="" required="required" />
</td>
</tr>
				<tr>
				  <td align="left" valign="top"><strong>File Number</strong><br />
  <select name="fileno" id="fileno" tabindex="4" style="width:400px">
    ...
    <?php
				...
				<tr>
				<td align="left" valign="top"><strong>Folio Code</strong><br />
				<select name="folio_code" id="folio_code" tabindex="5" style="width:400px">
					<option selected="selected" value="">---</option>
					<?php
					$r=@mysqli_query($con, "select *  from revenue_code where folio_code != '' order by reven_title");
					while ($rcourse=@mysqli_fetch_array($r))
						{
							$title=@$rcourse['reven_title'];$fcode=@$rcourse['folio_code']; $rcode=@$rcourse['rev_code'];
							$codeuse = $fcode."***".$rcode;
							echo "<option value='$codeuse'> $title<=>$fcode <=> $rcode</option>";
				...
				<tr>
				  <td align="left" valign="top"><strong>Amount (=N=)</strong><br />
				<input type='text' name='amount' class='amt' id='amount' value='' size='20'  style='background-color: #FEFFB0;font-weight: bold;text-align: right;' tabindex='6'></td>
  </tr>
				<tr>
				<td align="left"><strong>Remark</strong><br />
				  <textarea name="comment" cols="55" rows="3" id="comment" tabindex="7" ></textarea>				</td>
				</tr>
</table>
			</center>
			<div class="easyui-tabs" style="width:auto;height:auto"> <!-- begining of main tab -->
			 ...
			 <center>
				...
				<tr><th colspan="2">
				<input type="button" class="btn" name="sbtn" id="sbtn" value="Save" tabindex="9" onclick="swapcontent('posting_section2','save','Credit');" />
				<input type="button" class="btn" name="chbtn" id="chsbtn" value="Search" onclick="swapcontent('posting_section2','search','Credit');" />
				<input type="button" class="btn" name="chbtn" id="chsbtn" value="View All" onclick="swapcontent('posting_section2','view_all','Credit');" />
				<input type="button" class="btn" name="chbtn" id="chsbtn" value="Refresh" onclick="swapcontent('posting_section2','refresh','Credit');" />
				<!--,$('#ccode').val()-->
